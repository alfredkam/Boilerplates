...
$.initializeLogin = function() {
	...
	$.notification( 
		{
			title: "Welcome to AppFuel's Dashboard!",
	...
	$("#password button.login").bind("tap", function() {
		login();
	});
	...
	$("#password button.forgot").bind("tap", function(){
			$("#forgotFrame").show();
			$("#loginFrame").css({
				visibility : "hidden"
			});
	...
	$("#forgotFrame button.remindBtn").bind("tap", function(){
			forgot();
	});
	
	$("#forgotFrame button.backBtn").bind("tap", function(){
		$("#forgotFrame").hide();
		//("#loginFrame").show();
		$("#loginFrame").css({
			visibility : "visible"
	...
	function forgot() {
		...
		$.ajax({
			type: "POST",
			url: "/api/v2/security/password/reset",
			data: {
			...
			$.notification({
				title : "Success!",
				content : "If that email is in our system it will have had a new password sent to it.",
				border: false
			});
		});
	...
	function login() {
		//put login in shit here
		var email = $(".input.username input");
		...
		$.ajax({
			url: "/api/v2/security/login",
			type: "POST",
			data: dataToSend
			
		}).done(function(data){
			//simAlert.show("alert-success","Successfull login!");
			$.notification( 
					{
						title: "Success!",
						content: "Redirecting",
			...
			//simAlert.show("alert-warning","Login failed! Try again.");
			
			$.notification( 
					{
						title: "Login Failed",
						content: "Empty fields or Wrong credentials",
						icon: "!"
					}
				);
		}).always(function(){
...
$(document).ready(function() {
	var remember = $("input[type='checkbox']").click(function(){
		$(this).parents("span").toggleClass("checked");
	});
	//remember.parents("span").toggleClass("checked");
	
	//checkbox.parents("span").toggleClass("checked");
